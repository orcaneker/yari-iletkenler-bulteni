...
def gorsel_sec(r):
    """Exa sonucundan kullanılabilir görsel URL'si çıkar.

    Öncelik: extras.imageLinks (açıkça istenir) → üst düzey image alanı.
    Takip pikseli / logo / spacer gibi işe yaramaz görselleri ele.
    """
    adaylar = []
    ex = r.get("extras") or {}
    adaylar += [u for u in (ex.get("imageLinks") or []) if u]
    if r.get("image"):
        adaylar.append(r["image"])

    for u in adaylar:
        ul = u.lower()
        if not ul.startswith("http"):
            continue
        if any(x in ul for x in ("logo", "favicon", "sprite", "placeholder",
                                 "1x1", "pixel", "avatar", "icon", ".svg")):
            continue
        return u
    return None
...
def gorselleri_bagla(bulten, adaylar, olaylar=None):
    ...
    # olay bazlı yedek havuz: başlıktaki URL'lerden biri eşleşirse hepsi kullanılabilir
    olay_gorsel = {}
    for o in (olaylar or []):
        g = next((k.get("image") for k in o.get("kaynaklar", []) if k.get("image")), None)
        if g:
            for k in o.get("kaynaklar", []):
                olay_gorsel[k["url"]] = {"url": g, "credit": k["domain"], "type": "og"}
    ...
    log(f"Görsel bağlandı: {bagli} haber · görselsiz: {kaynaksiz}")
    return bagli
