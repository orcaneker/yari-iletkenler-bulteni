    ...
    for o in hedefler:
        ...
        if url not in mevcut_urller:
            adaylar.append({
                ...
                "published_date": yeni["published_date"], "image": r.get("image"),
...
# ============================================================
def gorselleri_bagla(bulten, adaylar):
    idx = {}
    for a in adaylar:
        if a.get("image"):
    ...
    for st in [bulten.get("lead")] + (bulten.get("stories") or []):
        if not st:
            continue
        adaylar_url = [(st.get("source") or {}).get("url")]
        adaylar_url += [k.get("url") for k in (st.get("supporting_sources") or [])]
        for u in adaylar_url:
            g = idx.get(url_normalize(u or ""))
            if g:
                st["image"] = g
                bagli += 1
                break
    log(f"Görsel bağlandı: {bagli} haber")
    return bagli
...
def main():
    ...
    gorselleri_bagla(bulten, adaylar)
