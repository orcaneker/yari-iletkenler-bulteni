...
# Görsel olmayan / logo / placeholder işaretleri — bunlardan biri geçen URL reddedilir
# Yalnızca KESİN logo/placeholder işaretleri. Fazla geniş tutmak gerçek
# haber görsellerini eler; bilinçli olarak dar tutuldu.
GORSEL_RED_IZLERI = (
...
def gorsel_gecerli(u):
    """Bir görsel GERÇEK haber görseli mi?

    Politika: yalnızca KESİN logo/placeholder işaretlerini ele. Kararsız
    kalınan her durumda görseli KORU — çalışan gerçek fotoğrafları
    yanlışlıkla elemektense, nadir bir logoyu kaçırmak yeğdir.
    """
    ...
    if not u.lower().startswith("http"):
        return False
    ul = u.lower().split("?")[0]
    if ul.endswith(".svg"):               # logolar/ikonlar neredeyse hep SVG
        return False
    # yol/dosya adında KESİN logo işareti varsa ele; onun dışında kabul et
...
def gorsel_sec(r):
    """Exa sonucundan GERÇEK haber görseli çıkar. Şüpheliyse None döner.

    Öncelik: extras.imageLinks (açıkça istenir) → üst düzey image alanı.
    imageLinks genelde makale içi görselleri verir; üst düzey 'image' bazen
    site logosu olabilir — bu yüzden ikincil sırada ve aynı filtreden geçer.
    """
    ex = r.get("extras") or {}
...
def odeme_duvarli(domain: str, metin: str) -> bool:
    """Alan adına DEĞİL, elimizdeki metne de bakarak karar ver.

    Exa bir kaynağın sadece teaser'ını görebiliyorsa, o metinle haber
    yazmak 'ödeme duvarı arkasında detaylandırılmadı' gibi içi boş
    cümlelere yol açıyor. Bunu kaynakta değil, PIPELINE'da çözüyoruz.
    """
...
def exa_ara(sorgu, dom_dahil, bas_tarih, bit_tarih, sonuc, konum=None, ek_disla=None):
    payload = {
        ...
        "contents": {
            ...
            # ⚠ Görsel için extras.imageLinks AÇIKÇA istenmeli.
            # Üst düzey "image" alanı her sonuçta dolmuyor.
            "extras": {"imageLinks": 2},
        },
    }
    if dom_dahil:
...
def _kelimeler(baslik):
    """Başlığı anlamlı kelime kümesine indirge."""
    t = (baslik or "").lower()
...
def teyit_ara(olaylar, adaylar):
    ...
    Bulunursa olay 'sadece_radar' olmaktan çıkar ve yazılabilir hale gelir,
    ancak 'ikinci_el' işaretlenir → metinde 'bildirildi' dili kullanılır.
    """
    ...
    for o in hedefler:
        ...
        # tarih penceresi: olay tarihinin ± toleransı
        try:
        ...
        # duvarlı kaynakları DIŞLAYARAK ara → sadece erişilebilir sonuç gelsin
        sonuclar = exa_ara(
            baslik, None, bas, bit, TEYIT["sonuc"],
        ...
        o["ikinci_el"] = True          # → prompt "bildirildi" dili kullanacak
        bulunan += 1
...
# 4-5) LLM ÇAĞRILARI
# ============================================================
def claude(model, sistem, kullanici, max_tokens, stream=False, cache=False):
    """Claude API çağrısı.
    ...
    """
    # Cache SADECE aynı sistem promptu birden çok kez gönderildiğinde işe yarar
    # (Haiku triyajı 9 partide çağrılıyor → cache okuma ~%90 ucuz).
    # TEK çağrılık Sonnet yazımında cache YAZMAK net ZARARDIR (%25 pahalı,
    # hiç okunmuyor) — o yüzden cache=False ile çağrılır.
    # ⚠ Minimum eşik ~1024 token: altındaki promptlar sessizce cache'lenmez,
    # bu yüzden triyajda önceki-olay listesi de sistem bloğuna konur.
    if cache and AYARLAR.get("prompt_cache"):
...
def triyaj(adaylar, bas, bit, state):
    ...
    # Sistem bloğu her partide AYNI → cache'lenir. Önceki olay listesi de
    # buraya konur: hem sabit, hem promptu 1024 token eşiğinin üstüne çıkarır.
    ...
    for i in range(0, len(adaylar), B):
        ...
        try:
            cikti = claude(
                ...
                cache=True,     # ← 9 partide aynı sistem promptu
            )
            d = json_ayikla(cikti)
...
def olaylari_zenginlestir(olaylar, adaylar):
    """Olaylara kaynak metinlerini bağla + BİRİNCİL KAYNAK ONARIMI.

    Ödeme duvarlı bir kaynak birincil seçilmişse ve olayda erişilebilir
    bir kaynak varsa, birincilliği ona devret. Tüm kaynaklar duvarlıysa
    olay 'sadece_radar' işaretlenir → yazılmaz, Radar'a düşer.
    """
    ...
    for o in olaylar:
        ...
        # Birincil seçimi: erişilebilir + düşük tier (1 = resmî/şirket) öncelikli
        ...
        if acik:
            ...
            # hiçbir kaynak açık değil → haber yazılamaz, Radar'a
            kaynaklar[0]["primary"] = True
            o["sadece_radar"] = True
            radara_dusen += 1
...
def dogrula(b):
    ...
    # --- şema kontrolü + kategori onarımı ---
    # Model bazen değer zinciri etiketini kategori sanıyor ("malzeme").
    # Sitede bozuk etiket görünmesin diye sessizce eşleştir.
    ESLESME = {
        "malzeme": "ekipman", "wafer-uretim": "yatirim", "eda-ip": "tasarim",
        "paketleme-test": "paketleme", "uretim": "yatirim", "cip": "ai-cip",
        "jeopolitik": "politika", "mevzuat": "politika", "piyasa": "rapor",
        "akademik": "rapor", "arastirma": "rapor", "guc-elektronigi": "guc",
    }
    for st in tum:
        ...
        if c not in KATEGORILER:
            ...
            if not yeni_c:
                # değer zincirinden tahmin et, olmazsa en genel kovaya at
...
# ============================================================
# 6.5) GÖRSEL BAĞLAMA
# Exa her sonuç için makalenin OG görselini döndürüyor. Sonnet bunu
# bilmez — Python'da kaynak URL'sinden eşleştirip story'ye bağlıyoruz.
# AI görseli ÜRETİLMEZ: teknik alanda uydurma görsel güvenilirliği bozar.
# ============================================================
def og_gorsel_cek(url):
    """Son çare: makalenin kendi OG görselini HTML'den çek.
    Exa bazı kaynaklar için (ör. CNBC) imageLinks döndürmüyor; bu durumda
    yalnızca öne çıkan haberler için sayfayı hafifçe çekip og:image okuruz.
    Ağ hatası olursa sessizce None döner — bülten etkilenmez.
    """
    try:
...
def gorselleri_bagla(bulten, adaylar, olaylar=None):
    """Haberlere görsel bağla.

    Sıra: (1) haberin kendi kaynak URL'lerinden, (2) o olayın DİĞER
    kaynaklarından. Birincil kaynakta görsel yoksa destekleyicininki de olur —
    aynı olayı anlatıyorlar.
    """
    idx = {}
    for a in adaylar:
        if a.get("image"):
    ...
    # olay bazlı yedek: olayın HERHANGİ bir kaynağında görsel varsa,
    # o olayın tüm kaynak URL'lerine ata (ödeme duvarlı birincil kaynağın
    # görseli olmasa bile destekleyicininki kullanılır).
    olay_gorsel = {}
    for o in (olaylar or []):
        g = next((k.get("image") for k in o.get("kaynaklar", []) if k.get("image")), None)
        if g:
            for k in o.get("kaynaklar", []):
                olay_gorsel[url_normalize(k["url"])] = {"url": g, "credit": k["domain"], "type": "og"}

    bagli, kaynaksiz = 0, 0
    for st in [bulten.get("lead")] + (bulten.get("stories") or []):
        if not st:
            continue
    ...
    # Son çare: hâlâ görselsiz öne çıkan haberler için OG etiketini çek
    # (en fazla 6 istek — maliyet/süre sınırı; radar zaten görselsiz)
    cekilen = 0
    for st in [bulten.get("lead")] + (bulten.get("stories") or []):
        if not st or (st.get("image") or {}).get("url") or cekilen >= 6:
            continue
    ...
    if kaynaksiz:
        eksikler = [((st.get("source") or {}).get("name") or "?")
                    for st in [bulten.get("lead")] + (bulten.get("stories") or [])
                    if st and not (st.get("image") or {}).get("url")]
        log(f"Görselsiz haberlerin kaynakları: {', '.join(eksikler)}")
...
def deploy(sayi_no):
    ...
    try:
        ...
        log("GitHub push başarılı → GitHub Pages 1-2 dk içinde yayına alır")
        return SITE_URL
    except Exception as e:
...
def main():
    ...
    gorselleri_bagla(bulten, adaylar, olaylar)
