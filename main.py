...
def dogrula(b):
    ...
    # brief string geldiyse nesneye çevir; geçersiz slug'ı temizle
    sluglar = {s.get("slug") for s in [b.get("lead")] + (b.get("stories") or []) if s}
    yeni = []
    for m in (b.get("brief") or []):
        if isinstance(m, str):
            yeni.append({"text": m, "slug": None})
        else:
            yeni.append({"text": m.get("text", ""),
                         "slug": m.get("slug") if m.get("slug") in sluglar else None})
    b["brief"] = yeni
